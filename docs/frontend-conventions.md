...
7개 도구는 앱 내부에 동기화된 `ToolHeader`를 사용하고 헤더를 브랜드·페이지 액션·유틸리티의 세 슬롯으로 구성한다. 런타임에는 저장소 루트 정본을 import하지 않는다. metadata의 `card` 값은 기존 생성 대상 분류이며 실제 화면은 평면 워크벤치다. `ToolHeader`가 가져오는 정본 `components/workbench.css`는 웹 도구에만 동기화하고 `home`과 Electron에는 적용하지 않는다.

`home`은 테마와 토큰 대상이지만 카드형 도구 셸 대상이 아니다. 평면형 sticky header와 Tool Hub master mark를 유지하며, 도구용 `ToolHeader` 생성물을 렌더하지 않는다.

| 슬롯 | 규칙 |
|---|---|
| 브랜드 | 뒤로 가기 아이콘 · Tool Hub · 세로 구분선 · 제품명. `h1`은 `--ds-font-size-title`, 모바일은 16px이며 긴 이름은 줄바꿈한다. **전체가 허브로 가는 링크**(`home`은 허브 자신이라 링크가 아니다) |
| 페이지 액션 | 헤더 아래 전체 폭 도구 모음. 앱 고유 액션을 놓고 비어 있으면 공간을 차지하지 않는다 |
...
- 375px·768px·1440px 모두 브랜드와 utilities가 첫 줄, actions가 그 아래 전체 폭에 놓인다. 모바일 액션은 줄바꿈하며 페이지 가로 스크롤을 만들지 않는다.
