...
기본 방향은 JSON에서 YAML이다. 툴바에는 `JSON → YAML`과 `YAML → JSON` 방향 선택기를 항상 제공한다. 사용자가 방향을 직접 선택하면 현재 원본 문자열을 유지한 채 선택한 형식으로 다시 해석하고 자동 변환한다. 따라서 빈 화면이나 문법 오류 상태에서도 YAML 입력 방향을 선택할 수 있다.

양쪽 편집기 사이의 방향 전환 버튼은 현재 결과를 새 원본으로 사용하는 별도 동작이다. 이 버튼은 현재 결과가 존재하고 최신 상태일 때만 활성화한다.
...
- 최대 중첩 깊이: JSON과 YAML 모두 collection 100단계
- 최대 생성 출력: UTF-8 기준 2MB. 변환, Pretty, 직접 직렬화 경로에서 동일하게 차단
...
입력 크기를 `n`, 생성 출력 크기를 `m`, 줄 수를 `l`, 최대 중첩 깊이를 `d`라고 할 때 복잡도는 다음과 같다.

- 파싱과 검증: 시간 `O(n)`
- 직렬화와 Pretty: 시간 `O(n + m)`. 출력의 이스케이프와 들여쓰기로 `m`이 `n`보다 커질 수 있으므로 입력만 기준 `O(n)` 보장은 하지 않는다.
- 줄 시작 offset 계산: 시간 `O(n)`, 공간 `O(l)`
- offset에서 행·열 조회: `O(log l)`
- 전체 작업 공간: `O(n + m)`, 재귀 보조 공간 `O(d)`
- 원본 변경 상태 전이: 공백 판별과 UTF-8 인코딩 때문에 시간 `O(n)`, 임시 공간 `O(n)`

깊이와 출력 제한을 넘거나 parser, 정규화기, 직렬화기가 예상하지 못한 예외를 던지면 작업을 중단하고 blocking 진단으로 변환한다. debounce timer 밖으로 예외를 전파하지 않는다.
