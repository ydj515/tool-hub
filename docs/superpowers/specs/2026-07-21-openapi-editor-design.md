...
Swagger Editor와 유사한 편집 경험을 Tool Hub에 추가한다. 사용자는 단일 Swagger/OpenAPI 문서를 YAML 또는 JSON으로 입력하고, 구조 탐색·검증·읽기 전용 API 문서 미리보기를 확인하며, Swagger 2.0과 OpenAPI 3.0/3.1/3.2 사이를 모든 방향으로 변환할 수 있다.
...
- YAML ↔ JSON 편집 포맷 변환
- 현재 편집 포맷과 독립적인 YAML·JSON 다운로드 및 버전별 YAML 예시 명세 다운로드
- Swagger 2.0, OpenAPI 3.0.x, OpenAPI 3.1.x, OpenAPI 3.2.x 입력 검증
- Swagger 2.0 ↔ OpenAPI 3.0 ↔ OpenAPI 3.1 ↔ OpenAPI 3.2의 모든 변환 방향
...
2026-07-23 범위 확장으로 OpenAPI 3.2.0을 포함한다. OpenAPI 3.2 전용 필드를 3.1 이하로 내릴 때는 `x-toolhub-original-*` 확장 보존과 JSON Pointer 손실 경고를 함께 제공한다.
...
| OpenAPI 3.2 | `openapi: "3.2.x"` |

지원 범위를 벗어난 값은 문서를 지우거나 강제로 낮추지 않고 `UNSUPPORTED_SPEC_VERSION` 오류를 표시한다.

### 3.2 변환 대상 버전

사용자에게 노출하는 대상은 다음 네 가지다.
...
| 3.1.x | 3.2.0 | 3.1 → 3.2 승격 규칙 → 재검증 |
| 3.2.x | 3.1.2 | 3.2 전용 필드 확장 보존·경고 → 재검증 |
| 3.2.x | 3.0.4 | 3.2 → 3.1.2 → 3.0.4 연쇄 변환 |
| 3.2.x | 2.0 | 3.2 → 3.1.2 → 3.0.4 → 2.0 연쇄 변환 |
...
type SpecFamily = 'swagger-2.0' | 'openapi-3.0' | 'openapi-3.1' | 'openapi-3.2';
...
- Swagger 2.0, OpenAPI 3.0.x, 3.1.x, 3.2.x를 검증하고 지원 범위 밖 버전을 명확히 거부한다.
- 지원 버전 간 모든 변환 경로가 구현되고 대상 버전 재검증을 수행한다.
- 하향 변환 손실이 JSON Pointer 경고로 나타난다.
- 설명되지 않은 변환 변경은 적용을 차단한다.
- 외부 `$ref`에 네트워크 요청을 하지 않는다.
- `Try it out`이 비활성화된다.
- 오류가 있어도 현재 원문과 마지막 유효 미리보기가 보존된다.
- YAML·JSON 다운로드와 버전별 YAML 예시 명세 다운로드 결과가 다시 파싱된다.
...
> Swagger 2.0과 OpenAPI 3.0/3.1/3.2는 표현력이 다르므로 모든 방향의 무손실 변환은 불가능하다. 대상 버전 검증 성공은 의미 보존 성공과 같지 않다.
>
