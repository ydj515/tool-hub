# openapi-editor Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Tool Hub에 YAML·JSON 자동 감지, Swagger/OpenAPI 전 버전 방향 변환, 손실 경고, 3분할 편집기와 읽기 전용 미리보기를 제공하는 openapi-editor를 추가한다.
...
- 최종 완료 전 `openapi-editor/`와 `home/`에서 `test`, `lint`, `typecheck`, `build`를 모두 실행하고 openapi-editor에서 `test:e2e`를 실행한다.
...
Expected: `openapi-editor` heading 또는 `문서 탐색기`를 찾지 못해 FAIL.
...
export default function Topbar(props: TopbarProps) {
  ...
  return (
    <header className="topbar">
      <div><h1>openapi-editor</h1><span>{props.filename} · {props.sourceVersion ? VERSION_LABEL[props.sourceVersion] : '버전 미감지'}</span></div>
      <div className="topbar-actions">
    ...
    <section aria-label="API 미리보기" className="preview-panel">
      ...
      <PreviewErrorBoundary>
        <SwaggerUI
          ...
          requestInterceptor={() => Promise.reject(new Error('openapi-editor는 외부 요청을 허용하지 않습니다.'))}
        />
      </PreviewErrorBoundary>
    </section>
  );
...
**Interfaces:**
- Consumes: 완료된 openapi-editor 동작과 검증 결과.
...
```ts
it('publishes openapi-editor as coming soon until deployment', () => {
  const tool = tools.find((item) => item.id === 'openapi-editor');
  expect(tool).toMatchObject({ name: 'openapi-editor', status: 'coming-soon', url: null });
...
Expected: 모든 명령 exit 0. openapi-editor 카드는 coming-soon이고 live URL 요구 테스트를 위반하지 않음.
...
```bash
rg -n "openapi-editor" README.md docs/contributor-guide.md home/src/data/tools.ts openapi-editor/README.md openapi-editor/docs/contributor-guide.md
...
git commit -m "docs(openapi-editor): document and register openapi-editor"
```
...
- openapi-editor의 `test`, `lint`, `typecheck`, `build`, `test:e2e`가 마지막 실행에서 모두 통과했다.
