...
- 입력 크기를 `n`, 생성 출력 크기를 `m`, 줄 수를 `l`, 최대 중첩 깊이를 `d`라고 할 때 파싱과 검증은 시간 `O(n)`, 직렬화와 Pretty는 시간 `O(n + m)`을 사용한다. 출력 이스케이프와 들여쓰기로 `m`이 `n`보다 커질 수 있으므로 입력만 기준으로 직렬화/Pretty를 `O(n)`이라고 보장할 수 없다.
- 파싱 트리와 결과 문자열을 포함한 전체 공간은 `O(n + m)`이며, 재귀 보조 공간은 `O(d)`이다.
- 줄 시작 offset 생성은 시간 `O(n)`, 공간 `O(l)`을 사용한다.
- offset의 행·열 조회는 정렬된 줄 시작 배열에서 이진 탐색해 `O(log l)`으로 처리한다.
- 원본 변경 React 상태 전이는 공백 판별과 UTF-8 인코딩 때문에 시간 `O(n)`, 임시 공간 `O(n)`이며, 실제 파싱·검증은 300ms debounce 뒤에만 `O(n)`으로, 직렬화·Pretty는 `O(n + m)`으로 실행한다.
- 브라우저 안전 경계는 JSON/YAML collection 중첩 100단계와 UTF-8 생성 출력 2MB이며, parse·정규화·직렬화·Pretty와 직접 `DataNode` 직렬화 경로에 적용한다.
- 테스트 fixture 전체 크기의 합을 `N`이라고 할 때 도메인 테스트의 총 파싱 작업은 `O(N)`이다.

> 주의사항: Monaco는 일반 textarea보다 초기 번들 비용이 크므로 로컬 worker chunk로 분리하고, 1MB 제한을 넘은 입력은 편집만 허용한다. 100단계 깊이 또는 2MB 출력 한계를 넘거나 내부 처리에서 예상하지 못한 예외가 발생하면 blocking 진단으로 중단한다. YAML 값 정규화는 주석과 anchor 표현을 의도적으로 제거하며, JavaScript `number` 정밀도를 넘어서는 수치의 완전 보존은 초기 범위에 포함하지 않는다.
