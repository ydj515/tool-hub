...
| 다국어 지원(한/영) | 웹 UI **및 산출물 본문(docx/xlsx/md)** 라벨을 한국어·영어로 제공. 사용자가 시작 시점에 언어 선택 | P0 |
...
GET /              → 업로드 폼 (UploadPage)
   첫 진입 시        → 언어 선택 모달이 화면 중앙에 표시(KO / EN)
                       선택 시 LOCALE 쿠키 저장 + 페이지 리로드
                       이미 쿠키가 있으면 모달 미표시
...
| 페이지 | 주요 컴포넌트 |
|---|---|
| `LanguageModal` (`templates/fragments/language-modal.html`) | 첫 진입 시 표시되는 Bootstrap 모달. KO / EN 두 버튼만. 선택 후 `LOCALE` 쿠키 저장하고 리로드 |
| `UploadPage` (`templates/upload.html`) | 좌: 폼(programName / version / **language(라디오, 현재 로케일로 사전 선택)** / formats 체크박스 / ZIP 드래그앤드롭), 우: 안내(허용 문자, 최대 크기, 멀티모듈 동작) |
...
| 기본 로케일 | `ko_KR` |
| 로케일 결정 우선순위 (웹 UI) | 1) 언어 선택 모달의 결과, 2) URL 쿼리 `?lang=ko|en`, 3) 쿠키 `LOCALE`, 4) HTTP `Accept-Language`, 5) 기본값 `ko_KR` |
...
- [ ] 첫 진입 시 언어 선택 모달이 표시되고, 선택 결과가 쿠키에 저장되어 재방문 시 모달이 표시되지 않는다.
